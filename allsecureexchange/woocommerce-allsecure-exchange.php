<?php
/*
Plugin Name: AllSecure Exchange - Recurring Latest
...
function load_textdomain_and_add_custom_order_status() {
	...
	register_post_status( 'wc-accepted',
		array(
			'label'                     => __( 'Allsecure Accepted (legacy)', 'allsecureexchange'),
			'public'                    => true,
			'exclude_from_search'       => false,
			'show_in_admin_all_list'    => true,
			'show_in_admin_status_list' => true,
			'label_count'               => _n_noop( 'Allsecure Accepted (legacy) <span class="count">(%s)</span>', 'Allsecure Accepted (legacy) <span class="count">(%s)</span>' )
		)
	);
}
...
function add_order_statuses( $order_statuses ) {
	...
	$order_statuses['wc-accepted'] = __( 'Allsecure Accepted (legacy)', 'allsecureexchange');
...
function woocommerce_allsecureexchange_init() {
	...
	class WC_AllsecureExchange extends WC_Payment_Gateway {
		...
		public function processTransaction($order, $token, $action, $installment_number='')
		{
			...
			if ($action == 'debit') {
				$transaction = new AllsecureDebit();
			} else {
				$transaction = new AllsecurePreauthorize();
			}
			...
			if( $is_recurring_duplicate ){
				...
				$this->log('Recurring Transaction');
			}
		...
		public function is_recurring_donation( $order_id ){
			...
			if( count($order_items) == 1 ){
				...
				if ($product && $product->is_type('donation') && get_post_meta( $product_id, '_recurring_donation', true ) === 'yes' && $order->get_meta('AS_RecurringDuplicate') !== 'yes') {
					return true;
				} else {
					return false;
				}
...
/**
 * Add 'Cancel recurring payment' order actions to 'My Account' page
 */
...
function as_my_account_orders_actions( $actions, $order ) {
	if ( $order->get_meta( 'AS_RecurringActive' ) == 'yes' ) {
		$actions['allsecure_cancel_recurring'] = array(
			...
			'name' => _x( 'Cancel monthly donation', 'Cancel recurring payment', 'allsecureexchange' )
		);
	}
	return $actions;
}
...
/**
 * Handling 'Cancel recurring payment' order actions
 */
...
add_action('woocommerce_account_monthly-donations_endpoint', function () {
	$orders = wc_get_orders([
		...
		// optionally: 'orderby' => 'date', 'order' => 'DESC',
	]);

	$customer_orders = array_values(array_filter($orders, function (WC_Order $order) {
		// If you truly only care that the key exists:
		return $order->meta_exists('AS_RecurringActive');
	}));
